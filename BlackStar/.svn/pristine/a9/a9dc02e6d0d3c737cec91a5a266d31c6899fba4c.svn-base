...
function pageDepth(pageNum) {
	...
	// 로그아웃
	if (pageNum == 3) {
		window.location.href = "/User/logout.do?logout=y";
	}
	// 인기상품
	if (pageNum == 4) {
...
function regist_func() {
	...
	$("#regBtn").click(
			function() {
				...
				document.getElementById("reg_frm").setAttribute("method",
						"post");
				document.getElementById("reg_frm").setAttribute("action",
						"/User/register.do");
...
function inquiryBook() {
	...
	$("#inquiryBtn").on("click",function() {
				...
				if (boardname.value == null || boardname.value.length <= 1) {
					...
					boardname.focus();
					return false;
				} else if (boardreason.value == null
						|| boardreason.value.length <= 1) {
					alert("사유를 다시 확인해주세요.");
					boardreason.focus();
					return false;
				} else if (boardcontent.value == null
						|| boardcontent.value.length <= 1) {
					alert("내용을 다시 확인해주세요.");
					boardcontent.focus();
					return false;
				}
	...
	$("#bookingBtn").on("click",function() {
						...
						var bookOutdate = document
								.getElementById("book_outdate");
						...
						if (bookOndate.value.length == 0) {
							...
							bookOutdate.focus();
							return false;
						} else if (Date.parse(bookOndate.value) > Date
								.parse(bookOutdate.value)) {
							alert("종료일이 시작일보다 길어야합니다.");
							return false;
						} else if (bookName.value == null
								|| bookName.value.length <= 1) {
							alert("이름을 다시 확인해주세요.");
							bookName.focus();
							return false;
						}
 ...
 * @returns
 */
function adminFunc(adminClickCnt){
	var adminfrm = document.getElementById("adminFrm");
	if(adminClickCnt == 1){
		//window.location.href="/Admin/adminInquiry.do";
	   adminfrm.setAttribute("action","/Admin/adminInquiry.do");
	   adminfrm.setAttribute("method","post");
	   adminfrm.submit();
	}
	else if(adminClickCnt == 2){
		//window.location.href="/Admin/adminBook.do";
	   adminfrm.setAttribute("action","/Admin/adminBook.do");
	   adminfrm.setAttribute("method","post");	   
	   adminfrm.submit();
	}
};
...
/**
 * 관리자전용 문의데이터, 예약문의 삭제 담당 함수
 * @param delValue
 * @returns
 */
function adminDel(delValue){
	window.location.href="/Admin/delInquiry.do?delValue=" + encodeURI(delValue);
};